...
{
  internal sealed class Pressurize : Card, IDougieCard, IHasCustomCardTraits
  {
    private static ISpriteEntry ThisArt = null!;
    public static void Register(IPluginPackage<IModManifest> package, IModHelper helper)
    {
      ThisArt = helper.Content.Sprites.RegisterSprite(package.PackageRoot.GetRelativeFile("assets/Card/Pressurize.png"));
      helper.Content.Cards.RegisterCard("Pressurize", new()
      {
        ...
        Name = ModEntry.Instance.AnyLocalizations.Bind(["card", "Pressurize", "name"]).Localize,
      });
    }
        ...
        ModEntry.Instance.KokoroApi.ActionCosts.MakeCostAction(
          ModEntry.Instance.KokoroApi.ActionCosts.MakeResourceCost(
            new CellResource(), 1
				...
				new AFAkePressurize { status = Status.droneShift, statusAmount = 3, targetPlayer = true}
			).AsCardAction
        ],
      Upgrade.B => [
...
public class AFAkePressurize : AStatus
{
    public override void Begin(G g, State s, Combat c)
    {
        c.QueueImmediate(new AStatus{ status = Status.droneShift, statusAmount = 3, targetPlayer = true});
