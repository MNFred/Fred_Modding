...
namespace Fred.Jack.cards
{
  internal sealed class RadarOverclock : Card, IJackCard
  {
    ...
    public override List<CardAction> GetActions(State s, Combat c) => upgrade switch
    {
      Upgrade.A => [
        new AStatus{
          status = Status.droneShift, statusAmount = 3, targetPlayer = true
        },
        new AStatus{
          status = ModEntry.Instance.MidrowHaltStatus.Status, statusAmount = 1, targetPlayer = true
        },
      ],
      Upgrade.B => [
        new AStatus{
        ...
        new AStatus{ status = ModEntry.Instance.LoseDroneshiftNextStatus.Status, statusAmount = 1, targetPlayer = true}
      ],
      _ => [
        new AStatus{
          status = Status.droneShift, statusAmount = 3, targetPlayer = true
